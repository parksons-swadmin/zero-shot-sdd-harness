...
## Sensitive Data
The uploaded AR file contains customer names and balances (commercially sensitive). In the **interactive app** it **never leaves the machine**: no network egress, no third-party API, no LLM. Logs record aggregate counts/timings only — never customer names, balances, or file contents. The upload temp file is deleted immediately after parse.

**The one authorized exception is the opt-in Phase-5 [scheduled email report](capabilities/scheduled_email_report.md):** when the user explicitly configures SMTP, that separate job emails the dashboard PDF (which contains customer names + balances) to the configured recipient — a deliberate, user-authorized egress. It is off by default (dry-run without SMTP), and its SMTP app password is a secret kept in `.env` (never committed, never logged). See [architecture.md](architecture.md#network-egress--scope-of-the-guarantee).
