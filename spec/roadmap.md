...
### Phase 3.2 (post-Phase-3 UI refinement) — On-demand data-quality audit + light/dark theme

- **Goal:** Two dashboard UI refinements. (1) The **risk-flags** (riskiest 90+ accounts) stay **inline**, but the detailed **data-quality audit** (per-row flagged-rows table + by-reason breakdown — the technical "which rows are messy" view) moves behind a **"Data-quality audit" button** that opens it on demand as a modal/drawer (an admin / spot-check view); the small **"N summary/total rows excluded"** tie-out note stays **visible inline**. (2) A **light/dark theme toggle** in the app header switches themes; the default follows the OS/browser `prefers-color-scheme`, and the user's explicit choice is remembered in `localStorage`. That theme flag is the **only** client-side persistence and holds **no AR data** — a UI preference, not business/data persistence, so the stateless / no-DB constraint is unaffected. All screens, charts and tables stay legible with WCAG-AA contrast in both themes; the print / PDF layout always renders in the light style.
- **Capabilities:** UI-only refinement — no new capability file; extends [proactive_flags](capabilities/proactive_flags.md) (audit now on-demand) and the Dashboard screen + global conventions in [ui.md](ui.md). **Backend unchanged** (the `data_quality_report` payload is identical; only its presentation changes).
- **Independent slices (parallel build units):**
  - `frontend` (frontend) — **deps: none.** Move the data-quality audit into an on-demand modal/drawer behind a "Data-quality audit" button (risk-flags remain inline; the "N summary/total rows excluded" note stays inline); add the header theme toggle with `prefers-color-scheme` default + `localStorage` persistence, and ensure both themes are legible (charts/tables/print). Files: `frontend/src/app/components/{FlagsPanel,DataQualityAudit,ThemeToggle}.tsx`, `frontend/src/app/globals.css`, `frontend/tests/e2e/phase32.spec.ts`.
- **Gate command:**
  1. from `frontend/`: `npx playwright test tests/e2e/ --reporter=line` — E2E after upload→map→compute on `ar_small.xlsx`: the risk-flags list is visible inline and the per-row audit table is **not** in the DOM until the **"Data-quality audit"** button is clicked, after which the modal/drawer lists the seeded flagged rows; the "N summary/total rows excluded" note is visible inline; toggling the header control switches `light`↔`dark` (asserts the theme attribute/class, and that a reloaded page restores the `localStorage` choice); `emulateMedia({media:'print'})` renders the light style regardless of the active theme.
  2. `cd frontend; pnpm build` — static export still builds clean.
- **How the user tests it (handoff seed):** rebuild + boot; upload `ar_small.xlsx`, confirm mapping. On the dashboard the riskiest-90+ accounts show inline; click **Data-quality audit** to open the per-row flagged-rows table + by-reason breakdown in a modal/drawer (close it again); the "N summary/total rows excluded" note stays visible. Click the header **theme toggle** to switch light↔dark (everything stays legible); reload — your chosen theme is remembered. Print / Save as PDF always comes out in the light style.

### Phase 4 — Invoice drill-down

- **Goal:** From the dashboard the user opens an inline drill-down section and inspects the individual invoices behind the aggregates — a searchable customer picker + employee filter (combining AND), a sortable per-invoice table (Customer · Invoice no. · Amount due · Due date · Days overdue · Aging bucket), and a filtered subtotal + invoice count computed over the full filtered set. Clicking an employee row in the employee-wise summary opens the section pre-filtered to that employee. Stateless, deterministic, no LLM. (This is a single-capability incremental enhancement phase, like Phases 3.1/3.2 — not a greenfield multi-capability phase.)
- **Capability:** [invoice_drilldown](capabilities/invoice_drilldown.md). *(Reuses the existing `Invoice` normalization + bucketing from Phases 1–2; no engine rewrite.)*
- **Independent slices (parallel build units):**
  - `drilldown-api` (backend) — **deps: none** (reuses the existing ingest→normalize→classify pipeline that already produces `Invoice` rows; builds against the [api.md](api.md)/[data.md](data.md) contract). Owns a pure filter/sort/cap/subtotal tool + the `POST /api/invoices` route. Files: `src/tools/drilldown.py` (new — filter by customer/employee, deterministic default sort, cap at `AGENT_DRILLDOWN_MAX_ROWS`, exact integer-paise subtotal + full `total_count`), `src/domain/aging.py` (+`InvoiceRow`/`DrilldownResult` models), `src/api/analysis.py` (+route), `src/config/settings.py` (+`AGENT_DRILLDOWN_MAX_ROWS`). May extend `tests/fixtures/expected_small.json` with a per-customer drill-down oracle block. Tests: `tests/unit/test_drilldown.py`, `tests/integration/test_drilldown.py`.
  - `frontend-phase4` (frontend) — **deps: none for build** (builds against the [api.md](api.md) contract); its Playwright E2E runs at phase assembly once `drilldown-api` is present. Owns the "Drill down" button + inline `DrilldownSection`, the type-to-search `CustomerPicker`, the employee filter, the sortable invoice table + filtered-subtotal line + truncation prompt, and wiring the employee-table row click to open the pre-filtered section. Files: `frontend/src/app/components/{DrilldownSection,CustomerPicker,InvoiceTable}.tsx`, `frontend/src/app/components/EmployeeTable.tsx` (add row-click handler), `frontend/src/app/page.tsx` (mount the section), `frontend/tests/e2e/phase4.spec.ts`.
- **Key surfaces / files:** disjoint (backend owns `src/`, frontend owns `frontend/`).
- **Gate command (all must pass; no LLM, no DB):**
  1. `uv run pytest tests/unit tests/integration -q` — backend over the real pipeline + real fixtures via TestClient:
     - `customer="Beacon & Co"` on `ar_small.xlsx` returns exactly that customer's real data rows (embedded summary/total rows excluded, its negative credit note **included**); `total_count` == that row count and `subtotal_amount` == the exact integer-paise sum of that customer's amount rows (`/100`), `truncated=false`.
     - adding `employee` narrows to the customer-AND-employee intersection; `employee="(blank)"` matches the seeded blank-employee row.
     - the seeded missing-due-date row returns `due_date=null`, `days_overdue=null`, `bucket="unclassified"`; a filter matching no rows returns `200` with `invoices=[]`, `total_count=0`, `subtotal_amount=0.0`.
